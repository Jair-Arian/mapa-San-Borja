/**
 * ============================================================================
 * MAPA SECTORIZADO DE SANBORJA — Application Logic
 * Sistema de Clasificación Territorial v4.0 (GeoPerú + Subsectores + Rutas)
 * ============================================================================
 *
 * FUENTE DE DATOS: data/AREA_SANBORJA.geojson y data/SANBORJA_SECTOR*.geojson
 */

document.addEventListener('DOMContentLoaded', () => {

  // ──────────────────────────────────────────────────────────────────────────
  // 1. POLÍGONOS DE LOS 9 SECTORES — Coordenadas GeoPerú Oficiales
  // ──────────────────────────────────────────────────────────────────────────

  const { sectors: SECTORS, subsectors: SUBSECTORS, area: AREA_SANBORJA } = window.SANBORJA_DATA;
  const SANBORJA_OFFICIAL_BOUNDARY = AREA_SANBORJA.features[0].geometry.coordinates[0].map(([lng, lat]) => [lat, lng]);
  const AREA_BOUNDS = L.latLngBounds(SANBORJA_OFFICIAL_BOUNDARY);
  const AREA_CENTER = AREA_BOUNDS.getCenter();
  const SEARCH_BOUNDS = {north: AREA_BOUNDS.getNorth(), south: AREA_BOUNDS.getSouth(), east: AREA_BOUNDS.getEast(), west: AREA_BOUNDS.getWest()};
  const UNASSIGNED = {id: 'sin-sector', name: 'Área sin sector asignado', shortName: 'Dentro de AREA_SANBORJA', color: '#64748B'};
  ...
  map.createPane('areaBoundary');
  map.getPane('areaBoundary').style.zIndex = 460;
  map.getPane('areaBoundary').style.pointerEvents = 'none';
  const districtOutline = L.polygon(SANBORJA_OFFICIAL_BOUNDARY, {
    pane: 'areaBoundary',
    color: '#2563EB',
  ...
  map.fitBounds(AREA_BOUNDS, {padding: [30, 30]});
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // 5B. DRAW SUBSECTOR POLYGONS (inner lines within sectors 1, 4, 6 and 9)
  // ──────────────────────────────────────────────────────────────────────────
   ...
   * e.g. "Av. Tomás Marsano 777, San Borja 15036, Perú" -> "Av. Tomás Marsano 777"
   */
   ...
   * Prevents returning generic names like "San Borja" or establishment names like "MINE DESIGN".
  ...
  function extractStreetAndNumberFromGoogle(results) {
    ...
    for (const r of results) {
      ...
      for (const comp of r.address_components) {
        if (comp.types.includes('route')) {
          ...
          if (route && route.toLowerCase() !== 'san borja' && route.toLowerCase() !== 'lima') {
    ...
    for (const r of results) {
      if (r.formatted_address) {
        ...
        if (cleaned && cleaned.toLowerCase() !== 'san borja' && cleaned.toLowerCase() !== 'lima' && !/^[A-Z0-9\+\s]+$/.test(cleaned)) {
          return cleaned;
        }
      }
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (p) {
          ...
          if (street) {
            return loc && loc !== street && loc !== 'San Borja' ? `${street} (${loc})` : street;
          }
  ...
  function isPointInPolygon(point, poly) {
    // Include edges and vertices, including the repeated closing vertex.
    for (let i = 0; i < poly.length - 1; i++) {
      const a = poly[i], b = poly[i + 1];
      const dx = b[0] - a[0], dy = b[1] - a[1];
      const length = Math.hypot(dx, dy);
      if (length && Math.abs(dx * (point[1] - a[1]) - dy * (point[0] - a[0])) / length < 1e-10 &&
          point[0] >= Math.min(a[0], b[0]) - 1e-10 && point[0] <= Math.max(a[0], b[0]) + 1e-10 &&
          point[1] >= Math.min(a[1], b[1]) - 1e-10 && point[1] <= Math.max(a[1], b[1]) + 1e-10) return true;
    }
    let inside = false;
  ...
  function isInsideArea(lat, lng) {
    return Number.isFinite(lat) && Number.isFinite(lng) && isPointInPolygon([lat, lng], SANBORJA_OFFICIAL_BOUNDARY);
  }

  function normalizeStops(stops) {
    if (!Array.isArray(stops)) return [];
    return stops.filter(s => s && isInsideArea(s.lat, s.lng)).map(s => {
      const sector = findSectorForPoint(s.lat, s.lng), sub = findSubsectorForPoint(s.lat, s.lng);
      return {...s, sectorId: sector.id, sectorName: sector.name, sectorShortName: sector.shortName, sectorColor: sector.color,
        subsectorId: sub?.id || '', subsectorName: sub?.name || '', subsectorShortName: sub?.shortName || '', subsectorColor: sub?.color || ''};
    });
  }

  function findSectorForPoint(lat, lng) {
    if (!isInsideArea(lat, lng)) return null;
    return SECTORS.find(s => s.polygon.some(p => isPointInPolygon([lat, lng], p[0]))) || UNASSIGNED;
  }

  /**
   * Find subsector for a given point. Checks only subsectors of the classified parent sector.
  ...
  function findSubsectorForPoint(lat, lng) {
    ...
    return SUBSECTORS.find(s => s.parentSectorId === sector.id && isPointInPolygon([lat, lng], s.polygon)) || null;
  }
  ...
  function addStopToRoute(addressName, lat, lng, sector) {
    if (!isInsideArea(lat, lng)) { showToast('La ubicación está fuera de AREA_SANBORJA.', 'error'); return; }
    sector = findSectorForPoint(lat, lng);
  ...
  map.on('click', async (e) => {
    ...
    if (!sector) {
      showToast('⚠️ La ubicación presionada está fuera de San Borja.', 'error');
      return;
    }
  ...
  function saveRouteToLocalStorage() {
    try {
      localStorage.setItem('sanborja_route_stops', JSON.stringify(routeStops));
    } catch (e) {
  ...
  function loadRouteFromLocalStorage() {
    try {
      const saved = localStorage.getItem('sanborja_route_stops');
      if (saved) {
        routeStops = normalizeStops(JSON.parse(saved));
        renderRouteOnMap();
        renderRouteStopsUI();
      }
    } catch (e) {
    ...
    if (activeModalTab === 'by-sector') {
      ...
      routeModalBody.innerHTML = `
        <div style="margin-bottom: var(--space-md); font-size: 0.8125rem; color: var(--text-secondary);">
          ...
          Sectores: <strong style="color:var(--text-primary);">${sectorsWithStops.filter(g => g.sector.id !== 'sin-sector').length} de 9</strong>
        </div>
  ...
  function copyRouteReport() {
    ...
    const grouped = {};
    [...SECTORS, UNASSIGNED].forEach(s => { grouped[s.id] = { sector: s, stops: [] }; });
    routeStops.forEach((stop, idx) => {
      if (grouped[stop.sectorId]) {
        grouped[stop.sectorId].stops.push({ ...stop, originalIndex: idx + 1 });
      }
    ...
    let report = `🗺️ RESUMEN DE RUTA EN SANBORJA\n`;
    report += `===================================\n`;
    report += `• Total de paradas: ${routeStops.length}\n`;
    report += `• Sectores recorridos: ${sectorsWithStops.filter(g => g.sector.id !== 'sin-sector').length} de 9\n`;
  ...
  function downloadRouteCSV() {
    ...
    link.href = url;
    link.download = `ruta_sanborja_${new Date().toISOString().slice(0, 10)}.csv`;
    link.click();
  ...
  async function shareRoute() {
    ...
    const shareText = `🗺️ Ruta en San Borja (${routeStops.length} paradas). Abre el enlace para verla en el mapa:`;

    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Ruta en San Borja',
          text: shareText,
          url: shareURL
        });
  ...
  function checkAndLoadSharedRoute() {
    ...
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        ...
        routeStops = normalizeStops(routeStops);
        saveRouteToLocalStorage();
        renderRouteOnMap();
        renderRouteStopsUI();
  ...
  async function handleSearch() {
    ...
    try {
      ...
      if (foundSector) {
        ...
        searchMarker.bindPopup(
          `<strong>${foundAddress}</strong><br>` +
          `<em>Ubicación fuera de los sectores de San Borja</em>`
        ).openPopup();
  ...
  function renderSuggestions() {
    ...
    suggestionsEl.innerHTML = currentSuggestions.map((s, i) => {
      ...
      if (!s.sector) {
        // Fuera de San Borja
        sectorBadge = `<span class="suggestion-sector">Fuera de San Borja</span>`;
      } else if (subsector) {
  ...
  function selectSuggestion(index) {
    ...
    if (!s || !isInsideArea(s.lat, s.lng)) return;
  ...
  if (SpeechRecognition) {
    ...
    recognition.onstart = () => {
      ...
      showToast('🎙️ Escuchando... Di tu dirección en San Borja', 'info');
    };
